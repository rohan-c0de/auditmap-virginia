...
- **Program Data (BSC)**: Extracted 157 programs from CourseLeaf catalog via table parsing (degree types: BAS, AA, AS, AAS, Dipl, Cert, COC; delivery methods)

## Future Work (Phases 2–3)

1. **Program Data (WSC, Dakota, NDSCS, LRSC)** 
   - WSC Acalog programs: Requires deeper catalog nav investigation (initial approach found 0 matches)
   - Dakota/NDSCS: Cleancatalog WAF-blocked on direct HTTP; requires Playwright
   - LRSC: PDF-only catalog; requires PDF extraction

2. **Fall 2026 Course Scrape** (attempted 2026-05-29, PS stability blocker)
...
3. **Cleancatalog Prereqs** (Dakota College at Bottineau, NDSCS; ~1.5 hrs)
