 ...
 * scrape-colleague.ts
 *
 * Scrapes course section data from MI community colleges that use
 ...
 *
 * Usage:
 *   npx tsx scripts/mi/scrape-colleague.ts                       # all colleges, auto-discover
 *   npx tsx scripts/mi/scrape-colleague.ts --college jackson-college
 *   npx tsx scripts/mi/scrape-colleague.ts --college delta-college --term "Fall 2026"
 *   npx tsx scripts/mi/scrape-colleague.ts --all
 */
...
// MI colleges using Ellucian Colleague Self-Service (10 confirmed publicly accessible)
...
async function main() {
  ...
  for (const [slug, baseUrl] of targets) {
    ...
    for (const currentTermName of termNames) {
      ...
      if (sections.length > 0) {
        ...
        // Some Colleague installations return term codes
        ...
        const outDir = path.join(process.cwd(), "data", "mi", "courses", slug);
  ...
  if (!args.includes("--no-import")) {
    ...
    await importCoursesToSupabase("mi");
  }
