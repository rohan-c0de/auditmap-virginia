 ...
 *
 * Output: data/mi/catalog-discovery.json (slug -> {platform, catalogUrl, via}).
...
const domains: Record<string, string> = JSON.parse(
  fs.readFileSync(path.join("scripts/mi/mi-college-domains.json"), "utf8")
);
...
(async () => {
  ...
  fs.writeFileSync("data/mi/catalog-discovery.json", JSON.stringify(results, null, 2));
  console.log("\nPlatform distribution:", JSON.stringify(byPlatform, null, 2));
  console.log("Wrote data/mi/catalog-discovery.json");
})();
