 ...
 *
 * Reads data/il/catalog-discovery.json (produced by discover-catalogs.ts),
...
async function main() {
  ...
  const disc: Discovery[] = JSON.parse(
    fs.readFileSync("data/il/catalog-discovery.json", "utf8")
  );
  ...
  const outDir = path.join(process.cwd(), "data", "il", "programs");
