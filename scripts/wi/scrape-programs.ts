/**
 * scrape-programs.ts — degree/program requirements for WI.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/wi/scrape-programs.ts
 */
...
async function run(
  ...
  try {
    ...
    const outDir = path.join(process.cwd(), "data", "wi", "programs");
...
async function main() {
  console.log("WI program scraper");
  // blackhawk-technical-college (courseleaf)
  await run("blackhawk-technical-college", () =>
    scrapeCourseleafPrograms({ collegeSlug: "blackhawk-technical-college", baseUrl: "https://catalog.blackhawk.edu" }),
  );

  // western-technical-college (courseleaf)
  await run("western-technical-college", () =>
    scrapeCourseleafPrograms({ collegeSlug: "western-technical-college", baseUrl: "https://catalog.western.edu" }),
  );
}
