 ...
 *
 * Output: data/tx/catalog-discovery.json (slug -> {platform, catalogUrl, via}).
...
const domains: Record<string, string> = JSON.parse(
  fs.readFileSync(path.join("scripts/tx/tx-college-domains.json"), "utf8")
);
...
(async () => {
  ...
  fs.writeFileSync("data/tx/catalog-discovery.json", JSON.stringify(results, null, 2));
  console.log("\nPlatform distribution:", JSON.stringify(byPlatform, null, 2));
  console.log("Wrote data/tx/catalog-discovery.json");
})();
