...
async function main() {
  ...
  const disc: Discovery[] = JSON.parse(fs.readFileSync("data/tx/catalog-discovery.json", "utf8"));
  ...
  console.log(`NC misc-platform program scraper — ${colleges.length} college(s)\n`);
  let total = 0;
