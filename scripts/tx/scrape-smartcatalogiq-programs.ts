/**
 * scrape-smartcatalogiq-programs.ts — NC SmartCatalogIQ program scraper.
 *
 * Reads data/tx/catalog-discovery.json (produced by discover-catalogs.ts),
...
async function main() {
  ...
  const disc: Discovery[] = JSON.parse(
    fs.readFileSync("data/tx/catalog-discovery.json", "utf8")
  );
  ...
  const outDir = path.join(process.cwd(), "data", "tx", "programs");
  fs.mkdirSync(outDir, { recursive: true });

  console.log(`NC SmartCatalogIQ program scraper — ${colleges.length} college(s)\n`);
  let totalPrograms = 0;
